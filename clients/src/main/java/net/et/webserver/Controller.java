...
public class Controller {
    ...
    @GetMapping(value = "/loans",produces = APPLICATION_JSON_VALUE)
    public List<StateAndRef<LoanState>> getLoans() {
        // Filter by state type: Loan.
        return proxy.vaultQuery(LoanState.class).getStates();
    }

    @PostMapping (value = "/create-loan" , produces =  TEXT_PLAIN_VALUE , headers =  "Content-Type=application/x-www-form-urlencoded" )
    public ResponseEntity<String> issueLoan(HttpServletRequest request) throws IllegalArgumentException {

        double amount = Double. valueOf(request.getParameter("loanValue"));
        ...
        // Create a new loan state using the parameters given.
        try {
            // Start the LoanIssueFlow. We block and waits for the flow to return.
            SignedTransaction result = proxy.startTrackedFlowDynamic(LoanFlow.Initiator.class, amount,otherParty).getReturnValue().get();
            // Return the response.
    ...
    /**
     * Displays all loan states that only this node has been involved in.
     */
    @GetMapping(value = "/my-loans",produces = APPLICATION_JSON_VALUE)
    public ResponseEntity<List<StateAndRef<LoanState>>> getMyLoans() {
        List<StateAndRef<LoanState>> myloans = proxy.vaultQuery(LoanState.class).getStates().stream().filter(
